...
/*   Updated: 2024/06/26 15:22:27 by marikhac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	data_init(t_terms *table)
{
	...
	safe_mutex_handle(&table->mtxtable_mutex, INIT);
	philo_init(table);
}
...
void	terms_parse(t_terms *table, char **argv)
{
	...
	if (table->time_to_die < (TIME_MIN * MILLISECONDS) || table->time_to_eat < (TIME_MIN * MILLISECONDS)
		|| table->time_to_sleep < (TIME_MIN * MILLISECONDS))
		{
		printf("%ld %ld %ld", table->time_to_eat, table->time_to_sleep, table->time_to_die);
